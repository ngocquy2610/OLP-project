class EnforceDigitsOnlyPhoneCheck < ActiveRecord::Migration[8.1]
  def up
    remove_check_constraint :users, name: "users_phone_format_check"

    add_check_constraint :users,
      "phone IS NOT NULL AND btrim(phone) <> '' AND char_length(phone) BETWEEN 10 AND 11 AND phone ~ '^\\d{10,11}$'",
