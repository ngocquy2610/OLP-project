...
ActiveRecord::Base.transaction do
  ...
  student = User.find_or_initialize_by(email: "student@gmail.com")
  ...
  # Use the teachers created above (or find existing teachers)
  all_teachers = User.where(role: :teacher).to_a

  ActiveRecord::Base.transaction do
    (1..100).each do |i|
    name = "Seeded Course #{i}"
    course = Course.find_or_initialize_by(name: name)
    course.user = all_teachers.sample
